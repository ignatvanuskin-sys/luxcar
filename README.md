...
Движение построено на `transform` и `opacity` (плюс одна маскированная
градиентная рамка), чтобы не грузить мобильные устройства:

* **Border beam** — светящаяся рамка (`.beam`): конический градиент в 1px-рамке,
  вращение через `@property --beam-angle`. Вне экрана анимация приостанавливается.
* **Орбы** — два размытых пятна, медленный дрейф, в hero и финальном CTA.
* **Счётчики** — `components/count-up.tsx` + `components/motion-observer.tsx`:
  числа набегают от нуля, когда попадают в вид. Значение отрисовано на сервере,
  поэтому SEO и режим без JS не страдают.
* **Hero-появление** — stagger-задержки `.rise` (бейдж → строки заголовка →
  абзац → кнопки → карточка рейтинга).
* **Микро-взаимодействия** — press-отклик (`.press`), встряска ошибки (`.shake`),
  блик на главной кнопке (`.sheen`), переходы шагов записи
  (`.step-forward` / `.step-back`), точечный лоадер (`.dot-loader`), тост в
  админке.

Всё отключается при `prefers-reduced-motion: reduce`. Подробности и источники
вдохновения — в [`AUDIT.md`](./AUDIT.md).
